...
export class MCPProvider extends CredentialProvider {
  ...
  async initialize() {
    try {
      ...
      console.warn('[MCPProvider] MCP Agent not reachable:', e.message);
    }
  ...
  async getCredentialMeta(label) {
    ...
    try {
      const response = await fetch(typeof window !== 'undefined' ? '/api/credentials/status' : 'http://localhost:3000/api/credentials/status', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'mcp', label })
      });
  ...
  async getStatus() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        return { ...baseStatus, details: data.details || 'MCP Agent reachable' };
      }
    } catch (e) {
      return { ...baseStatus, error: 'MCP Agent not reachable.' };
    }
