...
Install globally to get the `dos-browser`, `dos-browser-cli`, and `dos-browser-mcp` commands:

```bash
npm install -g dos-browser
```

…or run any interface instantly without installing, via `npx`:

```bash
npx dos-browser https://news.ycombinator.com/          # interactive TUI
npx -p dos-browser dos-browser-cli https://example.com # headless JSON
```

> **Requirements:** Node.js 18 or newer. The CLI transparently downloads a headless
> Chromium (via `puppeteer-extra`) the first time it needs JavaScript rendering.
...
Once installed from npm, the client can launch the server by command name — no
